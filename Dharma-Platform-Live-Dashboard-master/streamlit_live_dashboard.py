#!/usr/bin/env python3
"""
Streamlit Cloud Compatible Live Anti-Nationalist Dashboard
Simplified version for cloud deployment without heavy ML dependencies
"""
...
class StreamlitAntiNationalistDetector:
    """Streamlit Cloud compatible anti-nationalist content detector"""
    
    def __init__(self):
        self.initialize_session_state()
        
        # YouTube API key from Streamlit secrets or environment
        self.youtube_api_key = self.get_youtube_api_key()
    ...
    def get_youtube_api_key(self):
        """Get YouTube API key from Streamlit secrets or environment"""
        try:
            ...
            return 'AIzaSyAsiBWQPODfeRYuU2iKvh8CdpTvLGCUG50'
            
        except Exception as e:
            st.error(f"Error getting YouTube API key: {e}")
            return None
    ...
    def render_search_controls(self):
        ...
        with col2:
            if st.button("🔍 Search YouTube", type="primary"):
                if search_query.strip():
                    asyncio.run(self.search_youtube_content(search_query))
                else:
                    st.warning("Please enter search terms")
        
        with col3:
            if st.button("🎯 Auto Detect"):
                asyncio.run(self.auto_detect_threats())
        ...
        with col1:
            if st.button("Anti-India Propaganda"):
                asyncio.run(self.search_youtube_content("anti india propaganda"))
        
        with col2:
            if st.button("Modi Criticism"):
                asyncio.run(self.search_youtube_content("modi dictator fascist criticism"))
        
        with col3:
            if st.button("Kashmir Issue"):
                asyncio.run(self.search_youtube_content("kashmir human rights violation"))
        
        with col4:
            if st.button("Minority Issues"):
                asyncio.run(self.search_youtube_content("india minority persecution"))
    ...
    def analyze_threat_level(self, content: str, title: str) -> Dict[str, Any]:
        """Analyze threat level of content using simple keyword matching"""
        ...
        negative_words = ['hate', 'bad', 'corrupt', 'evil', 'terrorist', 'fascist', 'dictator', 'oppressive']
        positive_words = ['great', 'good', 'amazing', 'proud', 'strong', 'incredible', 'development']
        ...
        sentiment_score = (negative_count - positive_count) / max(1, negative_count + positive_count)
    ...
    async def auto_detect_threats(self):
        ...
        with st.spinner("Auto-detecting anti-nationalist content..."):
            ...
            for search_term in self.search_terms[:3]:  # Limit to avoid quota
                try:
                    results = await self.search_youtube_api(search_term, 10)
...
def main():
    ...
    detector = StreamlitAntiNationalistDetector()
